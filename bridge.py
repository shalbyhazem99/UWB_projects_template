...
def thread_log(complete_logs, log_folder, frequency, ser_model, start_bin, end_bin):
    ...
    for file_log in complete_logs:
        ...
        for i in range(size):
            ...
            for rx_index in range(3):
                ...
                for c in df_rx[rx_index][i][start_bin: end_bin if end_bin != -1  and end_bin < 120 else 120]:
                    frame += complex_to_4bytes(c)
            #frame += (b"\x00" * 2)  # header
            #frame += b"\n"

            ser_model.write(frame + b'\n')
...
def radar(radar_port, model_port, start_bin, end_bin):
...
def log(log_folder, frequency, model_port, start_bin, end_bin):
        ...
        while True:
            if ser_model.in_waiting > 0:
                ...
                if msg == "INFO":
                    ...
                    t1 = threading.Thread(target=thread_log, args=(complete_logs, log_folder, frequency, ser_model, start_bin, end_bin))

                    def wrapper():
                        try:
                            thread_log(complete_logs, log_folder, frequency, ser_model, start_bin, end_bin)
                        finally:
                            global thread_running
                            with thread_lock:
                                thread_running = False
...
if __name__ == "__main__":
    ...
    parser.add_argument("-S","--start_bin",type=int,required=True,help="First bin index")
    parser.add_argument("-E","--end_bin",type=int,required=True,help="Last bin index")
    ...
    if args.radar:
        radar(args.radar, args.model, args.start_bin, args.end_bin)
    else:
        log(args.log_folder, args.frequency, args.model, args.start_bin, args.end_bin)